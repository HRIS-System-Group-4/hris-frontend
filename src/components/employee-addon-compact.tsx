            ...
            <CardContent className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                    <span className="flex items-center gap-1">
                        Current: <strong className={"text-primary transition-all duration-500"}>{activeUser}</strong> /
                        <span className={additionalEmployees != 0 ? "text-muted-foreground" : ""}>
                            {currentLimit}
                        </span>
                        {/* {additionalEmployees != 0 && ( */}
                            <div className="flex items-center gap-1 transition-opacity duration-200" style={{ opacity: additionalEmployees > 0 ? 1 : 0 }}>
                    ...
                    <Badge variant={additionalEmployees === 0 ? "outline" : additionalEmployees > 0 ? "default" : "destructive"} className="text-xs">
                        {additionalEmployees >= 0 ? "+" : ""}{additionalEmployees} employees
                    </Badge>
                </div>

                <Slider value={[newLimit]} min={activeUser} max={maxEmployees} step={10} onValueChange={handleSliderChange} />

                {/* <div className="flex justify-between items-center">
                    <div className="text-right">
                        <div className="font-bold">${additionalCostMonthly.toFixed(2)}</div>
                        <div className="text-xs text-muted-foreground">additional/month</div>
                    </div>
                </div> */}
            </CardContent>
            {/* <CardFooter>
